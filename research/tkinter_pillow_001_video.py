## works
'''
This code loads a series of video frames as PIL Images, converts them to PhotoImages using the ImageTk.PhotoImage constructor, and displays them in a tkinter.Canvas widget using the create_image method.

The animate function updates the canvas with a new frame every 50 milliseconds, using the after method to schedule the next frame.
...
for i in range(8): 
	frame = Image.open(path + f"/image_sequence/Lisa_Turnaround_1920x1080_{i:04d}.png") 
	images.append(frame) 
